...
const SkillModal = ({ skill, onClose }) => {
  ...
  useEffect(() => {
    if (skill) setTimeout(() => setVisible(true), 10);
  }, [skill]);
  ...
  return (
    ...
    >
      <div className="relative bg-neutral-950 border border-orange-500/40 rounded-3xl p-10 max-w-md w-full text-center">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-8 mt-10 justify-items-center max-[350px]:grid-cols-1">
          {skillsData.map((skill, index) => (
            <ParallaxReveal
              key={index}
              effect="fadeDown"
              mobileEffect="fadeTop"
              speed={0.25}
              delay={index * 0.05}
            >
              <SkillCard skill={skill} onClick={setActiveSkill} />
            </ParallaxReveal>
          ))}
        </div>
      </div>
